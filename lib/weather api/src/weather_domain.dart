...
class Weather {
  ...
  Weather(Map<String, dynamic> jsonData) {
    ...
    Map<String, dynamic>? newTemp = jsonData['temp'];
    Map<String, dynamic>? newMinTemp = jsonData['min'];
    Map<String, dynamic>? newMaxTemp = jsonData['max'];
    ...
    _temperature = _unpackTemperature(main, 'temp').celsius == null
        ? _unpackTemperature(newTemp, 'temp')
        : _unpackTemperature(main, 'temp');

    _tempMin = _unpackTemperature(main, 'temp_min').celsius == null
        ? _unpackTemperature(newMinTemp, 'min')
        : _unpackTemperature(main, 'temp_min');

    _tempMax = _unpackTemperature(main, 'temp_max').celsius == null
        ? _unpackTemperature(newMaxTemp, 'max')
        : _unpackTemperature(main, 'temp_max');
...
List<Weather> _parseForecast(Map<String, dynamic> jsonForecast) {
  ...
  return forecastList.map((w) {
    // Put the general fields inside  every weather object
    w['name'] = name;
...
List<Weather> _parseHourlyForecast(Map<String, dynamic> jsonForecast) {
  List<dynamic> forecastList = jsonForecast['hourly'];
  Map<String, dynamic> test = forecastList[0];

  double? temperature = _unpackDouble(test, 'temp');
  double? clouds = _unpackDouble(test, 'clouds');

  return forecastList.map((e) {
    e['temp'] = {'temp': temperature};
    e['clouds'] = {'clouds': clouds};
    return Weather(e);
  }).toList();
}

List<Weather> _parseDailyForecast(Map<String, dynamic> jsonForecast) {
  List<dynamic> forecastList = jsonForecast['daily'];
  Map<String, dynamic> test = forecastList[0];
  Map<String, dynamic> temp = test['temp'];

  double? dailyTemp = _unpackDouble(temp, 'day');

  double? clouds = _unpackDouble(test, 'clouds');
  double? minTemp = _unpackDouble(temp, 'min');

  double? maxTemp = _unpackDouble(temp, 'max');

  double? rain = _unpackDouble(test, 'rain');
  double? snow = _unpackDouble(test, 'snow');

  return forecastList.map((e) {
    e['temp'] = {'temp': dailyTemp};
